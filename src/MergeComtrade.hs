...
mergeComtrade :: [FilePath] -> IO ()
mergeComtrade filenames = do
  mapM_ putStr $ "try to merge " : filenames
  ...
  --print $ length cdata
  --print cn
  let mergedFileName = takeBaseName $ head filenames
  writeComtradeData  (mergedFileName <.> "dat") cdn
  writeComtradeConf  (mergedFileName <.> "cfg") cfg
  return ()
